...
| 응답 상태 (`#serving`) | 외부에 잘 나가고 있나 | `_gw_request_log` (게이트웨이가 쌓는다) |
| API 사용량 (`#usage`) | 무엇이 얼마나 쓰이나 — API별·분야별 | `_gw_request_log` + `_catalog` |
| 이용자 키 (`#keys`) | 누가 쓰고 있고, 손댈 것이 있나 | `_keys` + `_usage` + `_gw_request_log` |
...
요청에 담긴 **값**과 응답 본문은 애초에 저장하지 않는다 — `_gw_request_log` 수집 원칙 ①(값이 아니라
...
- 삭제는 이용자의 삭제 요청을 운영자가 대신 처리하는 경로다. `_gw_request_log` 의 `key_hash`
...
`_gw_request_log` 가 저쪽에 쌓이기 때문이다 — 그래서 **`../marketplace` 를 먼저 시드**해야 한다.

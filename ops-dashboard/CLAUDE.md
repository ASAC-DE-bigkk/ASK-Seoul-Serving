...
- **하루 경계는 KST**: `date('now','+9 hours')` — 게이트웨이 `kstDay()` 와 같은 규약이어야
  쿼터 숫자가 맞는다.
- 🔴 **시간축은 KST 하나다 — 저장만 UTC 다** (시간축 감사 2차, 2026-08-12 · 팀 합의).
  파이프라인(ASAC-DAG/dbt)이 날짜 축을 KST 로 접기로 합의했고(`observed_date_kst`·
  `event_date`), 콘솔의 나머지 축이 **거기에 맞춘다** — 거꾸로 파이프라인을 바꾸지 않는다.

  |  | 규약 | 코드 |
  |---|---|---|
  | 저장 | UTC ISO — ⚠️ **마켓플레이스 소유의 보류 논점**(아래) | (마켓플레이스 `toISOString()`) |
  | 서빙 집계·날짜 필터 | KST 달력일 | `GW_DAY`(`date(ts,'+9 hours')`) · `dayWhere` |
  | 창 경계 | **KST 오늘 포함 N일** — 세 축(서빙·SLO·실행 기록) 동일 | `TS_SINCE` · `kstSince` |
  | 화면 날짜 접기·칩 | KST | `dayKey`(+9h) · `fillDays` KST 앵커 |
  | 시각 문자면 | **전부 KST** — `fmtTs` 하나(=`fmtTsKst`) | `kstMs` |

  ⚠️ **저장 시간대는 콘솔이 정하지 않는다** — 오너 판정(2026-08-12): 프로젝트 표기 표준이
  KST 인데 마켓플레이스가 UTC 로 적재하는 것은 **저쪽 결함으로 본다.** 다만 지금 저장을
  바꾸면 같은 컬럼에 두 시간대가 섞이는 순간이 진짜 왜곡이라 보류(발표 직전 무변경)했고,
  콘솔은 **적재분을 KST 로 해석해 읽는 것**으로 흡수한다. 저장이 KST 로 바뀌는 날이 오면
  콘솔의 접기 세 곳(`GW_DAY`·`TS_SINCE`·화면 `kstMs`)을 **같은 배포에서** 걷어야 한다 —
  한쪽만 걷으면 +9h 가 두 번 얹힌다.

  1차 감사 때는 "문자면이 그 표면의 축을 따른다"로 서빙만 UTC 로 남겼는데, 그 갈래 자체가
  이 감사에서 없어졌다 — 이제 **같은 시각은 화면 어디서든 같은 문자면**이다.
  🔴 시간대 표기 없는 문자열은 `kstMs` 가 **UTC 로 못 박는다** — `Date.parse` 에 그대로 주면
  브라우저가 로컬로 읽어(엔진 재량) 해외 브라우저에서 값이 틀어진다. 실측(2026-08-12)으로
  DAG·dbt 의 순간값 컬럼은 전부 오프셋 명시(`+00:00`)라 이 폴백을 타지 않는다.
  새 시각 컬럼을 읽게 되면 **먼저 실측**한다(`MAX(col)` 을 `datetime('now')` 와 견준다) —
  naive KST 로 적재되는 컬럼이 생기면 그 컬럼만 접기 없이 지나가게 해야 한다.
...
- 🔴 **서빙 로그의 분야는 `product_id` 밑줄 접두사이면서 `_ops_domain` 등록부에 있는 값만이다.**
  ...
  않는다(#63 · #156).
- 🔴 **이용자는 어느 화면에서든 같은 이름으로 부른다** (#326). 정본은 서버 `keymap()`
  (해시 앞 8자 → `key_prefix`·가린 이메일) + 화면 `userName()`/`hash8Name()` 한 벌이다.
  해시 앞 8자만 보여주면 '이용자 키' 화면의 `ask_…` 와 **눈으로 대조해야 같은 사람인지
  안다** — 실측 89.4%가 `_keys` 로 그대로 이어지는데 화면이 조인을 안 하고 있던 것이다.
  🔴 못 잇는 행은 **'삭제된 키'로 부르고 지우지 않는다** — 완전삭제로 대응이 사라지는 것은
  처리방침 제7조 ③이 약속한 상태라 못 잇는 게 맞다(빈칸은 '모른다'가 '없다'로 읽힌다).
  🔴 이메일 원문은 화면에도 응답에도 싣지 않는다 — 서버가 가려서 보낸다(`/api/v1/me` 만 예외,
  게이트웨이 규약). 🔴 **요청 로그에 이메일 해시를 실어 여정을 잇는 해법은 금지** —
  완전삭제 후에도 로그가 사람에 이어져 방금 한 약속을 깬다(#326 이 명시적으로 배제했다).
- 🔴 **'삭제된 키 제외'는 서버 필터다 — 화면 필터(DOMSCOPE)와 갈래가 다르다.**
  집계 카드(일자별·손님 구성·AI 수요·KPI)는 서버가 접어서 주므로 화면에서 못 거른다 —
  토글(`EXDEL`)이 `exclude_deleted=1` 을 실어 **다시 읽는다**(스코프는 재요청 없이 다시
  그림). 서버 술어는 이용 패턴 질의 6종(`exW`)에만 얹는다: 🔴 **identity 는 안 거른다**
  (뺀 몫을 셀 근거라서 — 0012) · **funnel 은 원래 `_keys` 를 돌아 삭제된 키가 안 들어간다** ·
  **익명은 남는다**(키가 없어 '삭제된 키'일 수 없다). 재방문 카드만 화면에서 거른다
  (`known` 플래그 — 서빙 구획 질의라 파라미터를 안 태웠고, 행 단위라 결과가 같다).
  버튼은 날짜 바 옆·이용 패턴 분석 탭에서만 보이고(딴 탭 지표는 안 타므로 보이면 "걸었다고
  믿는" 함정), 상태는 주소(`#usage?nodel=1`)에 실린다. §5 토글 규약 셋 준수.
...
- 분야 축을 고쳤으면 **'요청이 어느 분야로 세어지나' 카드의 다섯 갈래 합이 '전체 + 미등록 제외분'(meta.serving_unregistered_excluded)과 같은지**

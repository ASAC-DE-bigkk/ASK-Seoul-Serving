...
- **비밀값(`OPS_TOKEN` 등)은 `.dev.vars`.** 커밋·출력 금지. 위치·설정·권한 정본은 [../docs/secrets.md](../docs/secrets.md).
...
| 🔴 **시크릿·자격증명 값·위치·권한**(`OPS_TOKEN`·`CLOUDFLARE_*`·`.dev.vars`·`secret put`) | [../docs/secrets.md](../docs/secrets.md) **가 정본** — 위치·설정법·권한·배포 시크릿 성질은 거기만 고치고 다른 문서는 링크한다. 설명을 여러 문서에 복붙하지 않는다(그렇게 흩어졌던 걸 한곳으로 모았다) |

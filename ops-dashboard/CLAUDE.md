...
- 🔴 **`display` 를 주는 요소는 `ui.css` 에서 `[hidden]` 도 직접 꺼야 한다.**
  ...
  **화면에는 그대로 보였다.** 스코프바가 '이용 행동'·'이용자 키' 탭에서 안 사라진 원인이
...
- 🔴 **'보는 분야' 선택은 분야로 가를 수 있는 탭에만 노출한다.** 정본은 `SCOPED_PANES`
  **한 배열**이고, 지금 값은 `["pipeline","runs","serving","apis"]` 다 — **'이용 행동'·'이용자 키'
...
- 🔴 **분야는 `product_id` 의 밑줄 접두사가 있을 때만이다.** 없으면 **분야 미상**이고 아무
  분야에도 안 넣는다. `split("_")[0]` 만 쓰면 밑줄 없는 값이 통째로 분야가 되어
  K-Skill 번들(`seoul-urban-analytics`)·테스트 값(`not-a-v1-product`)이 **분야 드롭다운에
  항목으로 뜬다**(#162 실측). 화면 `domOf()` 와 서버 `GW_DOM` 은 **같은 규약**이어야 한다 —
  갈리면 화면이 세는 것과 서버가 센 것이 어긋난다. `table_name` 접두사는 **분야가 아니라
  등급**(`gold_`·`d1_`)이라 절대 쓰지 않는다(#63 · #156).
...
- 🔴 **'분야 미상'을 한 덩어리로 두지 않는다 — 읽는 사람의 할 일로 가른다.**
  ...
  `no_product`(목록·인증·키 발급) · `bundle` · `qa_probe` · `not_found`.

...
착수 전제 둘 — 어느 쪽도 설정 한 줄이 아니었다.

- ✅ **게이트웨이**: `[assets]` 에 `binding` 이 없던 것이 전제였다 — 그대로 `run_worker_first`
  에 더하면 워커가 그 경로를 아는 분기가 없어 problem+json 으로 떨어져 **파일이 깨진다.**
  **#238(charset 수정)이 `binding = "ASSETS"` 를 넣으면서 해소**됐고, `route="page"` 분기는
  #285 에서 붙인다.
- 🔴 **콘솔**: `route` 값 `page` 를 `decision/0014` 의 `ROUTE_KO` 에 **먼저** 올린다.
  `run_pattern` 때 배포가 반영보다 먼저 나간 사고(#132 → PR#138)가 같은 순서였다.
  ⚠️ `page` 는 **`SERVE_ROUTES` 에는 넣지 않는다** — 문서 접근은 데이터 서빙이 아니라
  관측 축이다(#177 §2 합의).

**관측 3경로 ≠ 워커 통과 4경로**(#285). `robots.txt` 는 charset 사유로 워커를 지나지만
**세지 않는다** — 통과 사유와 적재 사유를 섞으면 범위가 흐려지고, 크롤러가 주기적으로 치는
파일이라 "기계가 데이터 계약을 읽었다"는 신호가 묻힌다. 그리고 `page_path` 의 값은 이
3경로가 **전부**다(자유 문자열이 아닌 열거형) — 화이트리스트 밖 경로는 로그에 실리지 않는다.

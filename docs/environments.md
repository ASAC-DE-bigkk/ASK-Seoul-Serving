...
> ✅ **2026-08-06 — 갈라짐 해소.** dev D1 폐기(콘솔 decision/0015 · #85) 전환이 두 프로젝트
> 모두 끝났다. **세 환경(로컬·dev·prod)이 같은 운영 D1 을 본다.**
>
> | | 로컬이 보는 D1 | 배포 |
> |---|---|---|
> | **콘솔**(ops-dashboard) | 운영 `ask-seoul-prod-d1` | `ops.ask-seoul.kr` |
> | **게이트웨이**(marketplace) | 운영 `ask-seoul-prod-d1` | `dev.ask-seoul.kr` · `ask-seoul.kr` |
>
> 갈리는 것은 D1 이 아니라 **`ASK_ENV` 값과 배포 주소**뿐이다. 로컬 구동도 운영 데이터를
> 실제로 만지므로, 두 프로젝트 다 **바인딩에 `remote = true` 를 켜고 `--remote` 플래그는
> 쓰지 않는다**(플래그는 폐기 경로 — 붙은 것처럼 보이면서 질의만 비는 함정이 있다).

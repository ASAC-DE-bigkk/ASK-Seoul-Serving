...
## 0. 먼저 — 두 프로젝트가 **같은 D1** 을 본다

2026-08-05 에 잠깐 갈라졌었다 — 팀이 dev D1 을 폐기하고 전체 작업·검증·배포를 운영 D1
기준으로 하기로 했는데 **콘솔만 먼저 전환됐다.** 게이트웨이도 2026-08-06 에 따라오면서
**갈라짐은 닫혔다**(#85).

| | 로컬이 보는 D1 | 로컬에서 쓰기 | 배포 |
|---|---|---|---|
| **게이트웨이** `marketplace/` | **운영** `ask-seoul-prod-d1` | **운영에 그대로 간다** | `ask-seoul.kr` · `dev.` |
| **콘솔** `ops-dashboard/` | **운영** `ask-seoul-prod-d1` | **운영에 그대로 간다** | `ops.ask-seoul.kr` |

🔴 **갈리는 것은 D1 이 아니라 `ASK_ENV` 값과 배포 주소뿐이다.** 로컬 구동도 운영 데이터를
실제로 만진다 — 두 프로젝트 다 **바인딩에 `remote = true` 를 켜고 `--remote` 플래그는
쓰지 않는다**(플래그는 폐기 경로다 · 3-4절).

콘솔 쪽 정본은 [decision/0015](../ops-dashboard/docs/decision/0015-single-production-d1.md).

**절 구분은 남긴다** — D1 은 같아졌지만 명령·시크릿·배포면은 프로젝트마다 다르다.

| 절 | 다루는 것 |
|---|---|
| [1. 공통](#1-공통--두-프로젝트가-같이-지키는-것) | 설정 배치 · 시크릿 파일 · env 상속 · 마이그레이션 장부 |
| [2. 게이트웨이](#2-게이트웨이--marketplace) | 환경 3종(local·dev·prod) · 명령 · 원격 바인딩 |
...
| `ASK_ENV` | `local` | `dev` | `prod` |

🔴 **셋 다 같은 운영 D1 이다.** 갈리는 것은 `ASK_ENV` 값과 배포 주소뿐이다.

**"플래그가 없으면 로컬"이 여기서도 더는 성립하지 않는다.** 콘솔과 같은 방향이 됐다(3절) —
바인딩에 `remote = true` 가 박혀 있어 `npm run dev` 만으로 운영에 붙는다.

⚠️ **대가를 알고 쓴다.** 로컬 구동이 운영 `_keys`·`_usage`·`_burst` 를 실제로 건드린다.
테스트 키는 `DELETE /api/v1/keys?purge=true` 로 지운다. 요청 로그는 `ASK_ENV="local"` 로
찍혀 콘솔이 환경 스코프로 걸러낸다(#64) — 지표에 섞이지 않는다.

**왜 이렇게 했나**: dev D1 이 폐기돼 로컬이 볼 실물이 운영뿐이다. 서빙 메타(권리·컬럼
설명·활용 예시)가 운영에만 있어서 **로컬 픽스처로는 권리 게이트(#88)·카탈로그 응답을
검증할 수 없었다** — 실제로 #88 검증 때 `d1_catalog_sources` 를 손으로 만들어야 했다.

## 2-2. 사람이 치는 명령 — `seed` 가 없다

```bash
cd marketplace
npm install
npm run dev      # :8787 — 플래그 없이. 이것만으로 운영 D1 에 붙는다
```

`seed` 는 **없앴다** — 로컬이 실물을 보니 합성값을 넣을 자리가 없다. `fixtures/` 는 지우지
않고 남겼다(스키마가 어떤 모양이었는지 찾을 곳이 필요하다).
...
## 2-3. 🔴 원격에 붙는 방식 — **설정이지 플래그가 아니다**

콘솔이 먼저 밟은 함정을 그대로 승계한다(3-4절). `--remote` 플래그는 **폐기 경로**다.

| 방식 | 결과 |
|---|---|
| `wrangler dev` (바인딩 `remote = true`) | ✅ 정상 — 기동 로그에 `Mode: remote` |
| `wrangler dev --remote` (옛 플래그) | 배지는 운영인데 **질의만 빈 결과** |
| `experimental_remote` | 이 버전(4.115)에 **없는 필드** — 경고만 찍고 local 로 떨어진다 |
...
- ~~**두 프로젝트가 다시 한 규약으로 합쳐지는 시점**~~ — 2026-08-06 합쳐졌다(#85). 0절
  표를 갱신했고, **2·3절 분리는 남겼다** — D1 은 같아졌지만 명령·시크릿·배포면은 다르다.

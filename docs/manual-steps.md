...
| [4-1](#4-1-분야-등록부-_ops_domain-채우기) | 분야 등록부 `_ops_domain` | 콘솔 | ✅ 완료 (2026-08-07) — **새 D1 을 만들면 다시 해야 한다** |
...
콘솔의 **응답 상태 · 이용 패턴 분석 · API 사용량 · 이용자 키** 네 탭이 빈다.
...
## 4-1. 분야 등록부 `_ops_domain` 채우기

`_ops_slo`(4번)와 **다른 일이다.** 저쪽은 파이프라인이 넣는 측정값이고, 이건 콘솔이 소유한
**참조 내용**(분야 코드 → 한글 이름)이다. 마이그레이션은 표만 만들고 내용은 안 넣는다.

### 안 하면

화면이 분야를 **영문 코드로 부른다** — `culture`·`commerce`·`common`. 화면의 `domLabel()` 은
등록부에 없으면 코드를 그대로 쓴다(**라벨을 지어내지 않는 게 규약**이다). 탭 여섯 중 넷에서
새고, [CLAUDE.md §5](../ops-dashboard/CLAUDE.md)("화면 문구에 내부 용어를 쓰지 않는다") 위반이다.

**실제로 그 상태였다** — 운영 D1 등록부가 0행인 채로 있었다(2026-08-06 · 08-07 실측).
원인은 게으름이 아니라 **파일 구조**였다: 등록부가 `slo_live.sql`·`slo_sample.sql` 안에
들어 있었고 그 둘은 맨 앞에서 `DELETE FROM _ops_slo` 를 한다. D1 이 운영 하나뿐이라
(0015) sample 은 합성이라 못 돌리고 live 는 0005 가 폐기한 손 적재라, **등록부만 채울
방법이 없었다.** 2026-08-07 에 `fixtures/ops_domain.sql` 로 떼어내면서 풀렸다.
...
않는다"와 헷갈리지 않는다 — 그건 `slo_sample.sql` 얘기다).
...
🔴 **새 D1 을 만들면 다시 해야 한다.** 마이그레이션 장부에 안 남는 내용 적재라
`npm run migrate` 가 대신해 주지 않는다.

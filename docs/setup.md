...
marketplace 시드  →  ops-dashboard 시드  →  둘 다 dev
     (_keys · _gw_request_log 생성)   (_ops_slo 생성)
```
...
| `meta.missing`에 `_keys`·`_gw_request_log` | marketplace 미시드 **또는** 콘솔 `--persist-to` 누락 | marketplace `npm run seed` 먼저 (0절) / [environments.md §4-③](environments.md) |

...
- **키 원문은 어디에도 저장하지 않는다.** SHA-256 해시 + 표시용 접두 8자만. 발급 응답에서
  한 번 보여주는 게 전부다.
- **`_gw_request_log` 에 값을 남기지 않는다** — 필터는 컬럼명만, 식별자는 key_hash 만,
...
| `_issuance_log` | **여기** | 없음 |
| `_gw_request_log` | **여기** (`migrations/0002+`) | 읽기 전용 |
...
- 관측: 요청 후 `_gw_request_log` 에 행이 실제로 늘었는지 — **조용한 유실 검증, 생략 금지**.
...
_gw_request_log 컬럼 추가(#9·intent 축) → 새 ALTER 파일 + 시드 체인 + 콘솔 통지, 전부 nullable

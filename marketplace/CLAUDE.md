...
- **레이아웃·CSS 를 건드렸다면 `scripts/responsive-audit.js`** — 브라우저 콘솔에 붙여
  **320 · 375 · 768 · 960 · 1280** 다섯 폭에서 돌린다. 지표는 넷이다: 가로 밀림 · 입력
  요소 찌그러짐 · 부모 박스 넘침 · **포커스 확대 유발**(입력 글자 16px 미만 = iOS 확대).
  **밀림 하나만 보면 안 된다** — 카탈로그 결함 4건 중 3건이
  "페이지는 안 밀리는" 종류라 그 검사를 통과했었다(2026-08-07). 폭도 375·1280 둘만
  보다가 320 과 태블릿 구간(761~794 · 861~1042)을 통째로 놓쳤다.
  ④ 는 폭으로는 절대 안 잡힌다 — **확대가 렌더 뒤에 일어나** 감사에는 "밀림 없음"으로
  보인다. 그래서 전 화면 입력 22칸이 전부 걸린 채로 넉 달을 갔다(2026-08-13 전수).
  걸리는 것의 대부분은 원인이 하나다 — **flex/grid 항목의 `min-width:auto`**. 안 줄어드는
  항목이 보이면 조상에 `min-width:0` 또는 `minmax(0,1fr)` 을 먼저 의심한다.

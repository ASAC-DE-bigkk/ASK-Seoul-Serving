...
# E. 실사용 우수성 검증 — A/B (기계적 + 에이전트)

대시보드는 **표현력(가능/불가)** 을 정적 비교한다. "실제 사용에서 신설형이 좋은가"는 실제
질문을 던져 **결과 지표**로 재야 한다. 두 방식이 재는 게 다르니 함께 쓴다.

## E-1. 기계적 A/B — 마켓 랩 §5 패널 (자동 · AI 불필요)

마켓 콘솔 §5 **"실사용 A/B"** 카드: `product_id`·`pattern_id`(+필요시 `params`) 입력 → **A/B 실행**.
같은 질문을 `query_product`·`run_pattern` 으로 **실제 호출**해 지표를 나란히 본다.

| 지표 | 기존형(query_product) | 신설형(run_pattern) |
| --- | --- | --- |
| 이번 호출 반환 행 | 원시 N행(더 있음) | 답 M행 |
| 완전한 답에 필요한 전송 | 최대 제품 전량 · 여러 페이지 | 소량 · 1회로 끝 |
| 지연 · 쿼터 | 페이지당 1회 누적 | 1회 |
| 답 직결성 · 검증 | 원시 행·미검증 | 계산된 답·검증됨 |

기대: **효율·직결성 차이가 수치로** 드러난다. 🔴 동일 출처(마켓 랩) + API 키 필요.
`run_pattern` 은 MCP 전용이라 ops 대시보드(교차 출처)에서는 라이브 A/B 불가 → 마켓 랩에서.

## E-2. 에이전트 A/B — 실제 정답률 (MCP + 사람/LLM 채점)

진짜 "실사용"은 **AI가 실제로 답에 도달·정확한가**다. §D 로 MCP 를 붙인 뒤:

1. 벤치 질문 N개 — 카탈로그 `question_ko` 에서 능력별로 고르게(예: 정렬·집계·그룹 각 5개).
2. 각 질문을 **두 모드**로 AI(Claude)에게:
   - **AS-IS**: "query_product·preview·describe 만 써서 답해줘"
   - **TO-BE**: "run_pattern 을 우선 써서 답해줘"
3. **정답지(ground truth)** = `run_pattern` 의 검증 결과(`verified_rows`·`insight_sample_ko`).
4. 아래 채점표로 기록 → 집계.

### 채점표 (질문마다 한 줄)

| 질문 | 필요 능력 | AS-IS 답도달 | AS-IS 정확 | AS-IS 호출수 | TO-BE 답도달 | TO-BE 정확 | TO-BE 호출수 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| (예) 순유입 큰 업종? | sort·aggregate | △ | ✗ | 3 | ✓ | ✓ | 1 |
| … | … | ✗/△/✓ | ✗/✓ | n | ✓ | ✓ | 1 |

**집계**: 정답률 AS-IS x% vs TO-BE y% · 평균 호출수 · 오답/포기 비율.
**텔레메트리 교차확인**: 콘솔 '이용 행동'·'API 사용량' 탭에서 `agent_name/intent=max20_fable`
로 우리 트래픽의 호출수·오류율·행수·지연을 확인(§6).

-- _request_log 이름 충돌 검사 — 배포 전 필수 (ASAC-DAG#681 권고)
--
...
-- 389~401행이 7/21 부터 쌓이고 있다. 만든 주체는 미상 — 파이프라인·dbt·대시보드는
-- 전수 스캔으로 배제됐다(ASAC-DAG#681).
--
-- **사용법**: 배포 전 런북 0번에서 `--remote` 로 돌린다.
--   verdict 가 'OK' 가 아니면 **배포하지 말고** 이름 충돌부터 해소한다.
...
SELECT
  CASE
    ...
    WHEN (SELECT COUNT(*) FROM pragma_table_info('_request_log')
      ...
      THEN 'OK: 게이트웨이 스키마다'
    ELSE 'STOP: 다른 스키마의 표가 이름을 선점하고 있다 — 배포 전 해소 필요'
  END AS verdict,

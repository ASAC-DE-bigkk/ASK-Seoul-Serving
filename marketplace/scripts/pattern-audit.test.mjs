// P0 감사기 회귀·레드팀 — prep/pattern-audit.test.mjs(PR #232) 이식 + P0-a denyGate 검증.
//
// 두 게이트를 함께 고정한다:
//   denyGate(배선, P0-a)  — 내부/거부 표 참조·구조 위반만 거부. 형제·타 도메인 서빙표는 허용.
//   scopeGate(미배선, P0-b) — allowlist 밖 전부 거부. 게시 계약이 서면 배선한다.
// 공격 케이스는 prep 의 적대적 검증 3라운드(괄호 테이블·CTE 스코프 오염·차등 오라클) 확증본이다.
...
import {
  auditPatternSql, scopeGate, denyGate, collectDenied, tableRefs, tokenize, cteNames,
} from "../src/pattern-audit.js";

const HERE = dirname(fileURLToPath(import.meta.url));
const SNAPSHOT = join(HERE, "..", "fixtures", "catalog-snapshot.json");
const OK = new Set(["d1_ok", "d1_sibling"]);   // allowlist 모드에서 이 패턴이 읽어도 되는 테이블

// ── 레드팀: 내부표·구조 위반 페이로드 — **두 게이트 모두** 거부해야 한다 ────────────────
const ATTACKS = [
  ...
  // ── #217 P0-a: 결정 코멘트의 거부 목록 항목 ──
  ["마이그레이션 원장", "SELECT * FROM d1_ok, d1_migrations"],
  ["서비스 키 표", "SELECT * FROM d1_ok JOIN _service_keys ON 1=1"],
  ["발급 로그 표", "SELECT * FROM _issuance_log"],
];
for (const [name, sql] of ATTACKS) {
  test(`거부(allowlist·P0-b): ${name}`, () => {
  ...
  test(`거부(denylist·P0-a): ${name}`, () => {
    const gate = denyGate(sql);
    assert.equal(gate.ok, false, `막혀야 하는데 통과함: ${sql}`);
    assert.equal(gate.problem.status, 400);
    assert.equal(gate.problem.title, "pattern out of scope");
  });
}

// ── P0-a 와 P0-b 가 갈리는 지점: 타 도메인 서빙표 — 거부 목록은 **통과**(의도된 구분) ────
// 결정 코멘트: "거부 목록은 실수를 막지 침해를 막지 못한다" — 서빙표 간 경계는 P0-b 의 몫.
test("denyGate 는 타 도메인 서빙표를 막지 않는다 (P0-b 에서 막는다)", () => {
  const sql = "SELECT * FROM d1_ok JOIN d1_other_domain ON 1=1";
  assert.equal(denyGate(sql).ok, true);
  assert.equal(scopeGate(sql, OK).ok, false);   // allowlist 모드는 거부 — 2차 배선분
});

// ── 정상: 자기·형제 서빙표·json_each·CTE·자기조인 — **두 게이트 모두** 통과해야 한다 ────
const LEGIT = [
...
for (const [name, sql] of LEGIT) {
  test(`통과(allowlist): ${name}`, () => {
    const findings = auditPatternSql(sql, OK);
    assert.equal(findings.length, 0, `통과해야 하는데 막힘: ${sql}\n→ ${JSON.stringify(findings)}`);
  });
  test(`통과(denylist): ${name}`, () => {
    assert.equal(denyGate(sql).ok, true, `통과해야 하는데 막힘: ${sql}`);
  });
}
...
test("LIMIT :비표준이름 은 상한 미적용으로 findings (allowlist 감사 — 게시 린트용)", () => {
...
test("denyGate 는 LIMIT 이름을 문제 삼지 않는다 (게시 린트 소관 — 런타임 신규 거부 금지)", () => {
  assert.equal(denyGate("SELECT * FROM d1_ok LIMIT :maxrows").ok, true);
});
test("collectDenied: 변환 후(? 자리) SQL 에서도 동작한다 — 게이트는 converted 를 본다", () => {
  const toks = tokenize("SELECT * FROM d1_ok WHERE g = ? UNION SELECT k FROM _keys LIMIT ?");
  assert.deepEqual(collectDenied(toks), ["_keys"]);
});

// ── 회귀: 실제 카탈로그 431 패턴 (오탐 0 — 두 게이트 모두) ────────────────────────────
test("실측 431 패턴: allowlist 오탐 0 · denylist 오탐 0 · 내부표 참조 0", () => {
  const snap = JSON.parse(readFileSync(SNAPSHOT, "utf8"));
  const serving = new Set(snap.servingTables.map((t) => t.toLowerCase()));
  let total = 0, clean = 0, external = 0, denyClean = 0;
  for (const p of snap.products) for (const pat of p.patterns) {
    ...
    if (denyGate(pat.sql).ok) denyClean++;
  }
  assert.equal(total, 431);
  assert.equal(clean, 431, "실제 패턴이 allowlist 오탐으로 막히면 안 된다");
  assert.equal(denyClean, 431, "실제 패턴이 denylist 오탐으로 막히면 안 된다");
  assert.equal(external, 0);
});

test("회귀: 실제 패턴이 쓰는 테이블에 _keys 를 콤마조인하면 두 게이트 다 거부한다", () => {
  const snap = JSON.parse(readFileSync(SNAPSHOT, "utf8"));
  const serving = new Set(snap.servingTables.map((t) => t.toLowerCase()));
  let checked = 0;
  for (const p of snap.products) {
    for (const pat of p.patterns.slice(0, 1)) {
      ...
      if (!first) continue;
      checked++;
      const attack = `SELECT 1 FROM ${first}, _keys`;
      assert.equal(scopeGate(attack, serving).ok, false, `allowlist 주입 미검출: ${pat.pattern_id}`);
      assert.equal(denyGate(attack).ok, false, `denylist 주입 미검출: ${pat.pattern_id}`);
    }
  }

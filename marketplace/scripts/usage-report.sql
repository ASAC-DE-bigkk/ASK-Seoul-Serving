...
FROM _gw_request_log WHERE ts >= datetime('now', '-7 days')
...
FROM _gw_request_log WHERE ts >= datetime('now', '-30 days')
...
FROM _gw_request_log WHERE table_name IS NOT NULL AND ts >= datetime('now', '-30 days')
...
FROM _gw_request_log WHERE filters IS NOT NULL AND ts >= datetime('now', '-30 days')
...
FROM _gw_request_log WHERE status >= 400 AND ts >= datetime('now', '-30 days')
...
FROM _gw_request_log WHERE status = 200 AND row_count = 0 AND ts >= datetime('now', '-30 days')
...
FROM _gw_request_log WHERE key_hash IS NOT NULL

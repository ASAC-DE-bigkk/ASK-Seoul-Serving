...
FROM _gateway_request_log WHERE ts >= datetime('now', '-7 days')
...
FROM _gateway_request_log WHERE ts >= datetime('now', '-30 days')
GROUP BY day ORDER BY day DESC;

-- ③ 인기 제품 — 카탈로그 우선순위의 근거. preview 는 '구경', data 는 '실사용'이라 나눠 센다
-- 축은 **공개 식별자**다(decision/0003) — 물리 표명은 바뀔 수 있고 도메인마다 접두사가 갈린다.
-- product_id 가 NULL 인 행은 제품 해석 전에 끝난 요청이라(400·404) ⑤에서 센다.
SELECT '③ 제품별' AS report, product_id,
       SUM(route = 'preview') AS previews,
       SUM(route = 'data') AS calls,
       ROUND(AVG(row_count), 1) AS avg_rows
FROM _gateway_request_log WHERE product_id IS NOT NULL AND ts >= datetime('now', '-30 days')
GROUP BY product_id ORDER BY calls DESC, previews DESC;

-- ④ 필터 축 수요 — 어떤 컬럼으로 자르나. 계약에 인덱스·파티션을 어디 둘지의 근거
SELECT '④ 필터축' AS report, filters, COUNT(*) AS uses
FROM _gateway_request_log WHERE filters IS NOT NULL AND ts >= datetime('now', '-30 days')
...
FROM _gateway_request_log WHERE status >= 400 AND ts >= datetime('now', '-30 days')
...
FROM _gateway_request_log WHERE status = 200 AND row_count = 0 AND ts >= datetime('now', '-30 days')
...
FROM _gateway_request_log WHERE key_hash IS NOT NULL

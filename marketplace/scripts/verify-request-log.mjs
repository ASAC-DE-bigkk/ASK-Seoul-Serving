...
// 유실이 눈에 안 보이므로, 요청 N건 → _request_log 행 N건 증가를 실측으로 확인한다.
...
const cols = new Set(d1("PRAGMA table_info(_request_log)").map((c) => c.name));
if (!cols.has("request_id"))
  fail("_request_log 에 request_id 컬럼이 없다 — 시드 체인에 0004 미적용. " +
...
// 3) 판정
const after = d1("SELECT COUNT(*) AS n FROM _request_log")[0].n;
const delta = after - before;
if (delta !== 3)
  fail(`요청 3건을 보냈는데 로그가 ${delta}건 늘었다 (${before} → ${after}) — 조용한 유실. ` +
    "logRequest 의 INSERT 컬럼 목록과 로컬 스키마(마이그레이션·시드 체인)를 대조할 것");

const last = d1("SELECT route, status, request_id FROM _request_log ORDER BY ts DESC LIMIT 3");
